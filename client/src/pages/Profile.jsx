...
const Profile = () => {
  ...
  const handleListing = async () => {
    ...
    try {
      const res = await fetch(`api/user/show-listing/${currentUser._id}`)
      const data = await res.json()
  ...
  return (
    ...
    gap-4 pt-6 '>
      ...
      <p onClick={handleListing} className='text-green-700 cursor-pointer'>Show Listing</p>
      {listingsError && <p className='text-red-700'>{listingsError}</p>}
      {visible ? <h2 className='text-2xl font-semibold my-8'>Your Listing</h2> : ""}
